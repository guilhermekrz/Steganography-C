...
int EsconderP1::hide(Imagem* grande,Imagem* pequena)
{
    ...
    int lugarGrande = EsconderP1::pixelInicial(grande);
    if(lugarGrande==-1)
    {
        cout << "Não foi possível esconder a imagem utilizando a política 1" << endl;
        return 1;
    }

    EsconderP1::esconderAlturaLargura(pixelsGrande,lugarGrande, pequena->getAltura(),pequena->getLargura()); //8 pixels
    ...
    for(int i=0; i<alturaPequena; i++)
    {
        for(int j=0; j<larguraPequena; j++)
        {
            ...
            MPixels::esconderPixel(peq,pixelsGrande,lugarGrande); //8 pixels
        }
    }
...
void EsconderP1::esconderAlturaLargura(Pixel* pixelsGrande, int& lugarGrande,int alturaPequena,int larguraPequena)
{
...
int EsconderP1::pixelInicial(Imagem* grande)  //sequencia de 5 pixels iguais (iguais ao primeiro)
{
    SDL_LockSurface(grande->getSurface());
    Pixel* pixelsGrande = (Pixel*) grande->getSurface()->pixels;
...
//show
SDL_Surface* EsconderP1::show(Imagem* escondida)
{
    int alturaPequena = 0;
    ...
    int lugarGrande = EsconderP1::getPixelInicial(escondida);
    if(lugarGrande==-1)
    {
    ...
    int retal = EsconderP1::getAlturaLargura(pixelsGrande,lugarGrande,alturaPequena,larguraPequena);
    if(retal!=0)
    {
    ...
    for(int i=0; i<alturaPequena; i++)
    {
        for(int j=0; j<larguraPequena; j++)
        {
            ...
            int lugarPequena = i*larguraPequena+j;
            Pixel p = MPixels::pegarPixel(pixelsGrande,lugarGrande);
...
int EsconderP1::getPixelInicial(Imagem* grande)
{
    SDL_LockSurface(grande->getSurface());
    Pixel* pixelsGrande = (Pixel*) grande->getSurface()->pixels;
...
int EsconderP1::getAlturaLargura(Pixel* pixelsGrande, int& lugarGrande,int& alturaGrande,int& larguraGrande)
{
...
int EsconderP1::hidePintando(Imagem* grande,Imagem* pequena)
{
    SDL_LockSurface(grande->getSurface());
    SDL_LockSurface(pequena->getSurface());
